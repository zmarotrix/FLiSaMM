...
from resources import (
    CONFIG_FILE, APP_NAME, APP_VERSION, APP_DIMENSIONS,
    FL_ADJECTIVES, FL_NOUNS
)
from ui_components import CustomInputDialog, CustomMessageBox
from managers import PathManager, SaveProfileManager, ModManager
...
class FLiModManager(ctk.CTk):
    ...
    def __init__(self):
        ...
        try:
            self.logo_image = ctk.CTkImage(light_image=Image.open("resources/logo.png"), dark_image=Image.open("resources/logo.png"), size=(242, 80))
            self.steam_icon = ctk.CTkImage(light_image=Image.open("resources/steam.png"), dark_image=Image.open("resources/steam.png"), size=(24, 24))
    ...
    def _create_header(self):
        ...
        ctk.CTkLabel(header_frame, text=APP_NAME, font=ctk.CTkFont(size=20, weight="bold")).grid(row=1, column=0)
        
        self.game_status_label = ctk.CTkLabel(
            header_frame, text="GAME RUNNING - CONTROLS LOCKED",
            text_color="#E53935", font=ctk.CTkFont(weight="bold")
        )
    ...
    def _create_mod_management_tab(self):
        ...
        ctk.CTkButton(top_frame, text="Change...", width=80, command=self._prompt_for_game_path).grid(row=0, column=0, padx=(10, 5), pady=5)
        self.game_path_label = ctk.CTkLabel(top_frame, text="Game Path: Not Set", wraplength=600, justify="left", anchor="w")
        self.game_path_label.grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        self.install_mod_button = ctk.CTkButton(top_frame, text="Install Mod(s)...", command=self._select_mod_file)
        self.install_mod_button.grid(row=0, column=2, padx=(5, 10), pady=5)

        warning_frame = ctk.CTkFrame(tab, fg_color="#4A2B2B", border_width=1, border_color="#C75450")
        ...
        warning_label = ctk.CTkLabel(warning_frame, text=warning_text, wraplength=800, justify="left")
        ...
        eac_label = ctk.CTkLabel(utils_frame, text="Easy Anti-Cheat Bypass", font=ctk.CTkFont(size=14, weight="bold"))
        ...
        eac_desc = ctk.CTkLabel(utils_frame, justify="left", wraplength=800, text=eac_desc_text)
        ...
        self.eac_switch = ctk.CTkSwitch(utils_frame, text="Bypass Enabled", variable=self.eac_switch_var, onvalue="on", offvalue="off", command=self._toggle_eac_bypass)
        ...
        if self.steam_icon:
            ctk.CTkLabel(launch_sub_frame, image=self.steam_icon, text="").pack(side="left", padx=(0, 5))
        ...
        self.launch_steam_switch = ctk.CTkSwitch(
            launch_sub_frame, text="Launch via Steam", variable=self.launch_steam_var,
            onvalue="on", offvalue="off", command=self._on_launch_toggle_changed)
        self.launch_steam_switch.pack(side="left", padx=10)
        self.launch_button = ctk.CTkButton(launch_sub_frame, text="Launch Game", command=self._launch_game)
    ...
    def _first_launch_check(self):
        ...
        if config_path:
            ...
            if not is_valid:
                self._show_message("Path Invalid", "The saved game path is no longer valid.")
            ...
            if self.path_manager.find_game_automatically():
                ...
                self._show_message("Game Not Found", "Could not automatically detect the game directory. Please select it manually.")
                self._prompt_for_game_path()
        ...
        while True:
            ...
            if not path:
                if not self.path_manager.game_path:
                    self._show_message("Setup Required", "A valid game path is required to use the application.")
                    self.quit()
                return
            ...
            if is_valid:
                ...
                error_str = f"The selected folder failed validation:\n\n- {'.\n- '.join(errors)}"
                if self._ask_yes_no("Validation Failed", f"{error_str}\n\nThis may not be the correct folder. Use it anyway?"):
                    self.path_manager.set_game_path(path)
                    self._save_config()
                    self._update_all_paths_and_ui()
                    return
    ...
    def _toggle_eac_bypass(self):
        ...
        if target_state == "on":
            ...
            if not result["success"]:
                self._show_message("Error", f"Failed to apply EAC bypass: {result['error']}")
                self.eac_switch_var.set("off")
                return # Do not save config on failure
        else:
            result = self.path_manager.remove_eac_bypass()
            if not result["success"]:
                self._show_message("Error", f"Failed to remove EAC bypass: {result['error']}")
    ...
    def _launch_game(self):
        ...
        if self.launch_steam_var.get() == "on":
            ...
            eac_launcher_path = os.path.join(self.path_manager.game_path, "EACLauncher.exe")
            if os.path.exists(eac_launcher_path):
                ...
                self._show_message("Error", "EACLauncher.exe not found in the game directory.")
    ...
    def _rebuild_save_management_tabs(self):
        ...
        if self.save_tab_view.get():
            ...
            ctk.CTkLabel(
                self.save_master_tab, text="No Fantasy Life i save profiles found.",
                font=ctk.CTkFont(size=16)).place(relx=0.5, rely=0.5, anchor="center")
    ...
    def _populate_save_profile_tab(self, tab, profile_path):
        ...
        if manager.has_active_save_file() and not manager.metadata.get("active_slot_uuid"):
            manager.initialize_from_game_save(self._generate_random_name())

        tab.grid_columnconfigure(0, weight=1)
        tab.grid_rowconfigure(1, weight=1)
        widgets = {
            "selected_slot_uuid": None, "selected_backup_uuid": None,
            "backup_sort_order": tkinter.StringVar(value="date_desc"),
            "backup_search_term": tkinter.StringVar(), "slot_buttons": {},
            "backup_entry_pool": []
        }
        ...
        title_label = ctk.CTkLabel(
            title_frame, text=tab_name, font=ctk.CTkFont(size=14, weight="bold"),
            image=title_icon, compound="left", anchor="w")
        ...
        widgets["slot_list_frame"] = ctk.CTkScrollableFrame(left_panel, label_text="Save Slots")
        widgets["slot_list_frame"].grid(row=0, column=0, sticky="nsew", padx=5, pady=5)
        widgets["load_slot_button"] = ctk.CTkButton(
            left_panel, text="Load Slot", state="disabled", fg_color="gray",
            command=lambda: self._load_active_save_for_slot(profile_path, widgets["selected_slot_uuid"]))
        ...
        widgets["rename_slot_button"] = ctk.CTkButton(
            slot_actions, text="Rename Slot", state="disabled", fg_color="gray",
        ...
        widgets["copy_slot_button"] = ctk.CTkButton(
            slot_actions, text="Copy Slot to...", state="disabled", fg_color="gray",
            command=lambda: self._copy_slot(profile_path))
        widgets["copy_slot_button"].pack(fill="x")
        ctk.CTkButton(
            left_panel, text="[ + ] Create New Empty Slot",
        ...
        widgets["details_placeholder_frame"] = ctk.CTkFrame(details_panel, fg_color="transparent")
        ctk.CTkLabel(
            widgets["details_placeholder_frame"], text="Select a slot or create one.",
            wraplength=300).pack(expand=True, padx=20)
        ...
        search = ctk.CTkEntry(
            controls_frame, placeholder_text="Search manual backups...",
            textvariable=widgets["backup_search_term"])
        search.pack(side="left", padx=(0, 10), expand=True, fill="x")
        search.bind("<KeyRelease>", lambda e: self._refresh_backup_display(profile_path))
        ctk.CTkOptionMenu(
            controls_frame, variable=widgets["backup_sort_order"],
            values=["date_desc", "date_asc", "name_asc"],
            command=lambda e: self._refresh_backup_display(profile_path)
        ).pack(side="left")

        widgets["backups_list_frame"] = ctk.CTkScrollableFrame(content, label_text="Manual Backups")
        widgets["backups_list_frame"].grid(row=2, column=0, sticky="nsew", padx=10, pady=0)
        ...
        for _ in range(30):
            ...
            widgets["backup_entry_pool"].append({"frame": entry, "name_btn": name_btn, "load_btn": load_btn})
            entry.pack_forget()
        ...
        widgets["rename_backup_button"] = ctk.CTkButton(
            backup_actions, text="Rename Backup", state="disabled", fg_color="gray",
        ...
        widgets["delete_backup_button"] = ctk.CTkButton(
            backup_actions, text="Delete Backup", state="disabled",
        ...
        widgets["create_backup_button"] = ctk.CTkButton(
            backup_actions, text="Create New Backup", state="disabled",
            fg_color="gray", command=lambda: self._create_new_backup(profile_path))
    ...
    def _on_profile_tab_changed(self):
        ...
        if not tab_name:
            return
        profile_path = next((p for p, n in self.path_to_tab_name.items() if n == tab_name), None)
        if not profile_path:
            return
    ...
    def _on_slot_selected(self, profile_path, slot_uuid):
        ...
        widgets["load_slot_button"].configure(state="disabled" if game_locked else "normal")
        can_copy = any(p != profile_path for p in self.profile_managers.keys())
        widgets["copy_slot_button"].configure(state="disabled" if game_locked or not can_copy else "normal")

        is_loaded = (slot_uuid == manager.metadata.get("active_slot_uuid")) and manager.has_active_save_file()
    ...
    def _refresh_slot_list(self, profile_path):
        ...
        sorted_slots = sorted(
            ...
            key=lambda item: (item[0] != active_slot if is_loaded else True,
        ...
        for uuid, data in sorted_slots:
            ...
            if is_loaded and uuid == active_slot:
                name = f"{name} [LOADED]"
            btn = ctk.CTkButton(
                widgets["slot_list_frame"], text=name,
            ...
            fg_color = "#1F6AA5" if is_selected else \
                       ("#0E496D" if is_loaded and uuid == active_slot else "gray30")
            btn.configure(fg_color=fg_color)
            btn.bind("<Button-3>", lambda e, u=uuid, n=data.get('name'):
                     self._show_slot_context_menu(e, profile_path, u, n))
    ...
    def _populate_details_panel(self, profile_path):
        ...
        if selected_uuid and selected_uuid in manager.metadata["slots"]:
            ...
            is_loaded = manager.metadata.get("active_slot_uuid") == selected_uuid and manager.has_active_save_file()
            ...
            if active_ts:
                ...
                widgets["active_save_label"].configure(text="No active save for this slot.")
    ...
    def _refresh_backup_display(self, profile_path):
        ...
        if search_term:
            backups = [b for b in backups if search_term in b[1].get("name", "").lower()]
        ...
        for i, (backup_uuid, backup_data) in enumerate(backups):
            ...
            ts_str = datetime.fromtimestamp(backup_data['timestamp']).strftime('%Y-%m-%d %H:%M')
            ...
            entry["load_btn"].configure(
                ...
                command=lambda s=slot_uuid, b=backup_uuid: self._activate_backup(profile_path, s, b))
    ...
    def _update_ui_for_game_state(self, locked):
        ...
        if self.save_tab_view and self.save_tab_view.get():
            ...
            profile_path = next((p for p, n in self.path_to_tab_name.items() if n == current_tab), None)
    ...
    def _select_mod_file(self):
        ...
        for zip_path in filepaths:
            ...
            if not result["success"]:
                self._show_message("Installation Failed", result.get("error", "An unknown error occurred."))
        self._populate_mod_list()
    ...
    def _populate_mod_list(self):
        ...
        for i, mod_info in enumerate(mods):
            ...
            switch_var = tkinter.StringVar(value="on" if mod_status == "enabled" else "off")
            switch = ctk.CTkSwitch(
                row_frame, text="", variable=switch_var, onvalue="on", offvalue="off",
                command=lambda n=mod_name: (self.mod_manager.toggle_mod_status(n), self._populate_mod_list()),
                state="disabled" if self.is_game_running else "normal")
            switch.grid(row=0, column=0, padx=10, pady=10)
            ctk.CTkLabel(row_frame, text=mod_name, anchor="w").grid(row=0, column=1, sticky="ew", padx=10)
            delete_btn = ctk.CTkButton(
                row_frame, text="Delete", fg_color="#D32F2F", hover_color="#B71C1C", width=80,
    ...
    def _delete_mod(self, mod_name):
        ...
        if self._ask_yes_no("Confirm Deletion", f"Permanently delete the mod '{mod_name}' and all its files?"):
    ...
    def _load_active_save_for_slot(self, profile_path, slot_uuid):
        ...
        if self.is_game_running:
            self._show_message("Action Blocked", "Cannot load a different save slot while the game is running.")
            return

        if not slot_uuid: return
        ...
        if current_active_slot and current_active_slot != slot_uuid and dest_manager.has_active_save_file():
            dest_manager.save_active_game_state(current_active_slot)

        active_save_zip = os.path.join(dest_manager.slots_path, slot_uuid, "active_save.zip")
        ...
        if not os.path.exists(active_save_zip):
            ...
            self._show_message("New Save Slot Loaded", "The save directory has been cleared. Launch the game to create a new save file for this slot.")
    ...
    def _activate_backup(self, profile_path, slot_uuid, backup_uuid):
        ...
        if self.is_game_running:
            self._show_message("Action Blocked", "Cannot load a backup while the game is running.")
            return
        ...
        if current_active_slot and dest_manager.has_active_save_file():
            slot_name = dest_manager.metadata["slots"][current_active_slot].get("name", "Unknown")
            ...
            if self._ask_yes_no("Backup Current Save?", prompt):
                auto_name = f"Auto-Backup {datetime.now().strftime('%Y-%m-%d %H.%M.%S')}"
        ...
        backup_zip_path = os.path.join(dest_manager.slots_path, slot_uuid, f"{backup_uuid}.zip")
    ...
    def _create_new_slot(self, profile_path):
        ...
        dialog = CustomInputDialog(
            self, title="Create New Empty Slot", prompt="Enter a name for the new slot:",
            initial_value=self._generate_random_name(), random_name_func=self._generate_random_name)
        name = dialog.get_input()
        if name and name.strip():
            new_uuid = os.urandom(8).hex()
            # A new slot has no active save and thus no timestamp yet.
            manager.metadata["slots"][new_uuid] = {
                "name": name.strip(), "backup_counter": 1, "backups": {}}
    ...
    def _rename_slot(self, profile_path):
        ...
        dialog = CustomInputDialog(
            self, title="Rename Slot", prompt="Enter new name:",
            initial_value=current_name, random_name_func=self._generate_random_name)
    ...
    def _delete_slot(self, profile_path, slot_uuid):
        ...
        if not self._ask_yes_no("Confirm Deletion", f"Permanently delete slot '{slot_name}' and ALL its data?"):
            return

        is_loaded = manager.metadata.get("active_slot_uuid") == slot_uuid
        if is_loaded and self.is_game_running:
            self._show_message("Action Blocked", "Cannot delete the loaded slot while the game is running.")
            return
    ...
    def _create_new_backup(self, profile_path):
        ...
        if manager.create_new_backup(slot_uuid, auto_name):
            ...
            self._show_message("Error", "Could not create backup. Is a save currently loaded in this slot?")
    ...
    def _rename_backup(self, profile_path):
        ...
        current_name = manager.metadata["slots"][slot_uuid]["backups"][backup_uuid].get("name", "")
        dialog = CustomInputDialog(
            self, title="Rename Backup", prompt="Enter new name:",
            initial_value=current_name)
        new_name = dialog.get_input()
        if new_name and new_name.strip():
            manager.metadata["slots"][slot_uuid]["backups"][backup_uuid]["name"] = new_name.strip()
            manager._save_metadata()
            self._refresh_backup_display(profile_path)
    ...
    def _delete_backup(self, profile_path):
        ...
        backup_name = manager.metadata['slots'][slot_uuid]['backups'][backup_uuid].get('name', 'this backup')
        if self._ask_yes_no("Confirm Deletion", f"Permanently delete the manual backup '{backup_name}'?"):
            backup_zip = os.path.join(manager.slots_path, slot_uuid, f"{backup_uuid}.zip")
    ...
    def _copy_slot(self, source_path):
        ...
        if not destination_options:
            self._show_message("Copy Not Possible", "No other save profiles could be found to copy to.")
            return

        dialog = ctk.CTkToplevel(self)
        dialog.transient(self); dialog.title("Copy Slot to..."); dialog.geometry("400x150")
        dialog.resizable(False, False); dialog.grab_set()
        ctk.CTkLabel(dialog, text="Select destination profile:").pack(pady=(10, 5), padx=10)
        dest_var = tkinter.StringVar(value=list(destination_options.keys())[0])
        ctk.CTkOptionMenu(
            dialog, variable=dest_var, width=380,
            values=list(destination_options.keys())).pack(pady=5, padx=10, fill="x")
        user_choice = None
        def ok(): nonlocal user_choice; user_choice = dest_var.get(); dialog.destroy()
        def cancel(): dialog.destroy()
        btn_frame = ctk.CTkFrame(dialog, fg_color="transparent"); btn_frame.pack(pady=20, padx=10)
        ctk.CTkButton(btn_frame, text="OK", command=ok, width=100).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame, text="Cancel", command=cancel, width=100).pack(side="left", padx=10)
        dialog.protocol("WM_DELETE_WINDOW", cancel); self.wait_window(dialog)
        if not user_choice:
            return

        destination_path = destination_options[user_choice]
        dest_manager = self.profile_managers.get(destination_path, SaveProfileManager(destination_path))

        if slot_uuid in dest_manager.metadata["slots"]:
            existing_name = dest_manager.metadata["slots"][slot_uuid].get("name", "Unknown")
